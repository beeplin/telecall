...
function checkAndConvertRawConfig(rawConfig, directory) {
    var resourceMatcher = rawConfig.resourceMatcher, resolverBasePath = rawConfig.resolverBasePath, requestEndpoint = rawConfig.requestEndpoint;
    var config = {};
...
var toPosixPath = function (str) { return str.split(path_1.default.sep).join('/'); };
function mockResource(resourcePath, config) {
    var serverPath = toPosixPath(path_1.default.relative(config.resolverBasePath, resourcePath));
